$(document).ready(function () {
    renderEquipamentos();
    });
    
    function renderEquipamentos() {
      $('#tbodyid').html('');
      $.ajax({
        type: "POST",
        url: '../servidor/index.php/equipamentos',
        //O formato da resposta é Json:
        // {CD_Produto: "3", NM_Produto: "Óleo Shell Helix HX3 20W50", VR_Produto: "17,50", IM_Produto: "http://localhost/realcentercar/img/produtos/Oleo_20W50.jpg"}
        dataType: "json",
        success: function(data) {
          Variavelglobal=data
          console.log(data.length)
          Equipamentosmostrados = data.slice(0, 15)
          montatabela(Equipamentosmostrados);
          
          //tente logar data[1] ou data[1].NM_Produto ou data[2].IM_Produto
          //ProdutosTodos=data
          //QuantidadeDeTelas=Math.ceil(ProdutosTodos.length/ProdutosPorTela);
          //montarDivDePaginacao(QuantidadeDeTelas)
          //changeProdutosTodos(0);
        },
        error: function () {
          alert("Algum problema com a conectividade com o banco.");
        }
      }).responseText; 
    }
    
    function getid(){
      $.ajax({
        type: "POST",
        url: 'servidor/index.php/id',
        dataType: "json",
        success: function(data) {
          console.log(data);
        }
      })
    }
    
    function montatabela(Equipamentosmostrados) {
      $('#tbodyid').html('');
      Equipamentosmostrados.forEach(function (obj, index) {
        var p1 = '<tr meuid="' + index + '"><td>' + obj.CD_EQPT + '</td>'
        var p2 = '<td>' + obj.NM_EQPT + '</td>'
        var p3 = '<td>' + obj.DC_TIPO_EQPT + '</td>'
        var p4 = '<td>' + obj.QT_MOTOR + '</td>'        
        var p5 = '<td>' + obj.IC_TIPO_PRPS + '</td>'
        var p6 = '<td>' + obj.QT_PSGR + '</td>'
        var p7 = '<td><a href="#deleteEmployeeModal" class="delete" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="Delete">&#xE872;</i></a></td>'
        //<td><a href="#deleteEmployeeModal" class="glyphicon glyphicon-ok" data-toggle="modal" >'
        var p8 = '</a></td></tr>'
        var finaldiv = p1 + p2 + p3 + p4 + p5 + p6 + p7 + p8;
    
        $('#tbodyid').append(finaldiv);
    
        //$( "tbody#tbodyid tr:eq(5)" ).css( "backgroundColor", "#ff0" );
      
      });
    }
    